...
function downloadTemplate() {
  const headers = [
    ...
    "lease_start (optional)",
    "lease_end (optional)",
    "rent_due_day (optional)",
    "payment_method (optional)",
    "card_expiry (optional)",
    "late_payment_count (optional)",
    "days_late_avg (optional)",
    "previous_delinquency (optional)",
    "notes (optional)",
  ]
  const example = [
    "Jane Smith",
  ...
  a.href = url
  a.download = "rentsentry_tenant_template.csv"
  a.click()
...
export default function UploadPage() {
  ...
  async function buildPreview() {
    ...
    if (!isHistorical && rawRows.length > 200) {
      toast.warning(`Your CSV has ${rawRows.length} rows — only the first 200 will be imported. Split the file into batches of 200 to import all tenants.`)
    }
    ...
    <div>
          ...
          <div className="mt-4 flex items-center gap-3 bg-[#0d1117] border border-white/[0.06] rounded-xl px-5 py-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[#4b5563] text-xs mt-0.5">
                ...
                All other fields are optional but improve risk scoring and automation accuracy.
              </p>
            </div>
            <button
              onClick={e => { e.stopPropagation(); downloadTemplate() }}
              className="shrink-0 flex items-center gap-1.5 text-xs font-semibold text-[#60a5fa] bg-blue-500/10 border border-blue-500/20 px-3 py-2 rounded-lg hover:bg-blue-500/20 transition-colors"
            >
              <Download size={12} />
              Download template
            </button>
      ...
      {step === "mapping" && (
        <div className="bg-[#131929] border border-[#1e2d45] rounded-xl p-6">
          ...
          <p className="text-[#9ca3af] text-sm mb-4">Match your spreadsheet columns to RentSentry's fields. Only <span className="text-white">name</span> and <span className="text-white">unit</span> are required — everything else is optional but improves risk scoring and automation accuracy.</p>
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            {Object.entries(mapping)
              ...
              .map(([field, col]) => {
                const required = field === "name" || field === "unit"
                return (
                <div key={field} className="flex items-center gap-3">
                  <span className="text-[#9ca3af] text-xs w-36 shrink-0 capitalize">
                    {field.replace(/_/g, " ")}
                    {!required && <span className="text-[#374151] normal-case"> (optional)</span>}
                  </span>
                  <select
                    value={col || ""}
      ...
      {step === "preview" && (
        <div>
          ...
          <div className="bg-[#131929] border border-[#1e2d45] rounded-xl p-6 mb-6">
            ...
            {(() => {
              const overwriteCount = [...changeMap.values()].filter(v => v.type !== "new").length
              return overwriteCount > 0 ? (
                <div className="flex items-start gap-2 bg-orange-500/10 border border-orange-500/20 rounded-lg px-4 py-2.5 mb-4">
                  <AlertTriangle size={13} className="text-orange-400 shrink-0 mt-0.5" />
                  <span className="text-orange-300 text-xs">
                    <span className="font-semibold">{overwriteCount} existing tenant record{overwriteCount !== 1 ? "s" : ""} will be replaced.</span>
                    {" "}Payment and activity history is preserved, but current tenant data (balance, rent, lease dates) will be overwritten with this CSV. Make sure this file is up to date.
                  </span>
                </div>
              ) : null
            })()}
